...
class Graph {
	 ...
	 * 
	 * @param u
	 *          u is the source or the root of the DFS Tree.
	 */
		...
		for (Edge e : u.getAdjecencies()) {
			...
			if (!v.isVisited()) { // unvisited Edges
				...
				if (u.getParent() == null) {
					...
					if (childrenCount > 1) {
						// In DFS Tree, if a root have 2 or more children, then it is the
						// articulation point
					...
					if (v.getShortestTime() >= u.getDiscoveryTime()) {
						// In DFS Tree, no descendant of u which have a tree edge to any of
						// the ancestors of u,
						// however when there exists a back edge, the shortest vertex from
						// the root will be the articulation point
				...
				if (v != u.getParent()) { // v is not the previous node
					...
					// hence set the shortest time being the ancestors (v) only if it is
					// smaller.
...
class Edge {
	...
	/***
	 * Using Builder Pattern to create Edge object. It allows us to have a
	...
	public static class Builder {
		 ...
		 * 
		 * @param vertex
		 *          this is the vertex to which the edge connecting to.
		 */
...
class HospitalTour {
	...
	void run() throws Exception {
		...
		while (TC-- > 0) {
			...
			// read rating scores, A (index 0), B (index 1), C (index 2), ..., until
			// the V-th index
